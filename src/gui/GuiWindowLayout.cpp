...
GuiWindow::Layout
GuiWindow::computeLayout(const int screenWidth, const int screenHeight,
  ...
  float sideWidth =
      GuiWindowInternal::clampFloat(availableWidth / 5.5F, 200.0F, 290.0F);
  ...
  layout.commandRect =
      Rectangle{outerPadding, screenHeight - outerPadding - commandHeight,
                screenWidth - outerPadding * 2.0F, commandHeight};
  const float contentBottom = layout.commandRect.y - gap;

  layout.leftPanelRect = Rectangle{outerPadding, contentTop, sideWidth,
                                   contentBottom - contentTop};
  layout.rightPanelRect =
      Rectangle{screenWidth - outerPadding - sideWidth, contentTop, sideWidth,
                contentBottom - contentTop};
  ...
  const float rosterHeight = GuiWindowInternal::clampFloat(
      layout.rightPanelRect.height * 0.40F, 220.0F, 336.0F);
  layout.rosterRect = Rectangle{
  ...
  const float buttonWidth =
      (layout.commandRect.width - buttonPadding * 2.0F - buttonGap * 2.0F) /
      3.0F;

  for (int index = 0; index < 6; ++index) {
    const int row = index / 3;
    const int col = index % 3;
    layout.quickButtonRects[static_cast<std::size_t>(index)] = Rectangle{
        layout.commandRect.x + buttonPadding + col * (buttonWidth + buttonGap),
        layout.commandRect.y + buttonPadding + row * (buttonHeight + rowGap),
        buttonWidth, buttonHeight};
  }

  layout.scrollLeftRect = Rectangle{layout.commandRect.x + buttonPadding,
                                    layout.commandRect.y + 72.0F, 30.0F, 20.0F};
  layout.scrollRightRect = Rectangle{
      layout.commandRect.x + layout.commandRect.width - buttonPadding - 30.0F,
      layout.commandRect.y + 72.0F, 30.0F, 20.0F};
  ...
  if (currentSnapshot.gameStarted) {
    const Rectangle boardSurface =
        GuiWindowInternal::insetRect(layout.boardRect, 10.0F, 10.0F);
...
void GuiWindow::updateFrame(const Layout &layout,
  ...
  if (currentSnapshot.gameOver && currentSnapshot.hasWinnerSummary) {
    ...
    if (!popupDismissed) {
      if (GuiWindowInternal::isButtonPressed(gameOverPopupExitButtonRect(),
                                             true)) {
      ...
      if (GuiWindowInternal::isButtonPressed(gameOverPopupNewGameButtonRect(),
                                             true)) {
      ...
      if (IsKeyPressed(KEY_ENTER) || IsKeyPressed(KEY_ESCAPE) ||
          GuiWindowInternal::isButtonPressed(gameOverPopupCloseButtonRect(),
                                             true)) {
        std::lock_guard<std::mutex> lock(snapshotMutex);
        gameOverPopupDismissed = true;
        return;
  ...
  const Vector2 mouse = GetMousePosition();
  {
    ...
    const Rectangle textRect{
        layout.logRect.x, layout.logRect.y,
        std::max(20.0F, layout.logRect.width - logScrollbarWidth - 10.0F),
        layout.logRect.height};

    const std::string logText =
        GuiWindowInternal::buildTransactionLogText(currentSnapshot);
    const auto lines = GuiWindowInternal::wrapText(
        georgiaFont, logText, 15.0F, 1.0F, textRect.width, 100000);
    ...
    const Rectangle scrollbarTrack{
        ...
        layout.logRect.y, logScrollbarWidth, layout.logRect.height};
    const Rectangle scrollbarThumb = GuiWindowInternal::computeScrollbarThumb(
        scrollbarTrack, visibleLines, static_cast<int>(lines.size()),
        logScrollLine);
    ...
    if (logScrollbarDragging) {
      if (IsMouseButtonDown(MOUSE_LEFT_BUTTON)) {
        ...
        logScrollLine = GuiWindowInternal::clampInt(
            static_cast<int>(std::round(ratio * maxStartLine)), 0,
            maxStartLine);
        logAutoScroll = logScrollLine >= maxStartLine;
      } else {
  ...
  if (GuiWindowInternal::pointInsideRect(layout.rosterRect, mouse) &&
    ...
    for (std::size_t i = 0; i < currentSnapshot.players.size(); ++i) {
      const Rectangle cardRect = GuiWindowInternal::calculateRosterCardRect(
  ...
  if (currentSnapshot.gameStarted && !currentSnapshot.gameOver) {
    ...
    if (GuiWindowInternal::isButtonPressed(layout.scrollLeftRect,
    ...
    if (GuiWindowInternal::isButtonPressed(layout.manualButtonRect,
                                           manualEnabled)) {
...
void GuiWindow::updateModalInput() {
  ...
  if (wheel != 0) {
    std::lock_guard<std::mutex> lock(modalMutex);
    modalScrollOffset = GuiWindowInternal::clampFloat(modalScrollOffset - wheel,
                                                      0.0F, modalScrollMax);
  }
  ...
  if (modalDragging) {
    if (IsMouseButtonDown(MOUSE_LEFT_BUTTON)) {
      modalPosition =
          Vector2{mouse.x - modalDragOffset.x, mouse.y - modalDragOffset.y};
      ...
      modalPosition.y = GuiWindowInternal::clampFloat(
          modalPosition.y, 10.0F,
          GetScreenHeight() - dialogRect.height - 10.0F);
    } else {
      modalDragging = false;
    }
  ...
  bool isUsername = false;
  {
    std::lock_guard<std::mutex> lock(snapshotMutex);
    isPlayerCount =
        (!snapshot.gameStarted && snapshot.startupMode == "PLAYER_COUNT");
    isUsername = (!snapshot.gameStarted && snapshot.startupMode == "USERNAME");
  }

  if (isPlayerCount) {
    float centerY = dialogRect.y + 250;
    Rectangle minusRect = {dialogRect.x + dialogRect.width / 2 - 120,
                           centerY - 30, 60, 60};
    Rectangle plusRect = {dialogRect.x + dialogRect.width / 2 + 60,
                          centerY - 30, 60, 60};
    ...
    if (GuiWindowInternal::isButtonPressed(minusRect, true)) {
      ...
      if (pCount > 2)
        modal.inputText = std::to_string(pCount - 1);
      else
        modal.inputText = "2";
      return;
    }
    ...
    Rectangle okBtn = {dialogRect.x + dialogRect.width / 2 + 10,
                       dialogRect.y + dialogRect.height - 80, 140, 50};
    Rectangle cancelBtn = {dialogRect.x + dialogRect.width / 2 - 150,
                           dialogRect.y + dialogRect.height - 80, 140, 50};
    ...
    Rectangle okBtn = {dialogRect.x + dialogRect.width / 2 + 10,
                       dialogRect.y + dialogRect.height - 80, 180, 50};
    Rectangle cancelBtn = {dialogRect.x + dialogRect.width / 2 - 190,
                           dialogRect.y + dialogRect.height - 80, 180, 50};
    ...
    if (GuiWindowInternal::isButtonPressed(cancelBtn, true)) {
      ...
      return;
             ...
             currentModal.prompt.find("[PROPERTY_PURCHASE:") !=
                 std::string::npos) {
    Rectangle yaBtn = {dialogRect.x + dialogRect.width / 2 + 10,
                       dialogRect.y + dialogRect.height - 80, 140, 50};
    Rectangle tidakBtn = {dialogRect.x + dialogRect.width / 2 - 150,
                          dialogRect.y + dialogRect.height - 80, 140, 50};

    if (GuiWindowInternal::isButtonPressed(yaBtn, true) ||
        IsKeyPressed(KEY_Y)) {
      std::lock_guard<std::mutex> lock2(modalMutex);
      modal.response.accepted = true;
    ...
    if (GuiWindowInternal::isButtonPressed(tidakBtn, true) ||
        IsKeyPressed(KEY_N)) {
      std::lock_guard<std::mutex> lock2(modalMutex);
      modal.response.accepted = true;
    ...
    while (std::getline(ss, line)) {
      if (line.find(". ") != std::string::npos &&
          line.find("(Tile ") != std::string::npos)
    ...
    for (size_t i = 0; i < candidates.size(); ++i) {
      Rectangle itemR = {gridX + (i % 2) * (itemW + 30),
                         gridY + (i / 2) * (itemH + 30), itemW, itemH};
    ...
    Rectangle targetBtn = {dialogRect.x + dialogRect.width - 250,
                           dialogRect.y + dialogRect.height - 85, 210, 55};
    if (GuiWindowInternal::isButtonPressed(targetBtn,
                                           !currentModal.inputText.empty())) {
      confirmLocalDialog();
    }
    return;
    ...
    Rectangle contentArea = {dialogRect.x, dialogRect.y + 120, dialogRect.width,
                             dialogRect.height - 190};
    ...
    for (const auto &line : lines) {
      if (line.find(". ") != std::string::npos) {
        Rectangle itemRect = {dialogRect.x + 25, itemY, dialogRect.width - 50,
                              90};
        if (GuiWindowInternal::isButtonPressed(itemRect, true)) {
          std::lock_guard<std::mutex> lock2(modalMutex);
    ...
    Rectangle okBtn = {dialogRect.x + dialogRect.width - 160,
                       dialogRect.y + dialogRect.height - 80, 130, 50};
    ...
    return;
             ...
             currentModal.prompt.find("UPGRADE HOTEL") != std::string::npos) {
    // Upgrade Hotel
    Rectangle yaBtn = {dialogRect.x + 60, dialogRect.y + 520, 250, 80};
    Rectangle tidakBtn = {dialogRect.x + dialogRect.width - 310,
                          dialogRect.y + 520, 250, 80};
    if (GuiWindowInternal::isButtonPressed(yaBtn, true)) {
      std::lock_guard<std::mutex> lock2(modalMutex);
      modal.response.value = "y";
      modal.response.accepted = true;
      ...
      modal.active = false;
      modalCondition.notify_all();
    } else if (GuiWindowInternal::isButtonPressed(tidakBtn, true)) {
      std::lock_guard<std::mutex> lock2(modalMutex);
      modal.response.value = "n";
      modal.response.accepted = true;
      modal.backendResolved = true;
    ...
    return;
             ...
             currentModal.prompt.find("DemolitionCard") != std::string::npos) {
    ...
    Rectangle confirmBtnDemolish = {
        listX, dialogRect.y + dialogRect.height - 100, listW, 80};
    if (GuiWindowInternal::isButtonPressed(confirmBtnDemolish,
                                           !currentModal.inputText.empty())) {
      confirmLocalDialog();
    }
    return;
  } else if (currentModal.active &&
             (currentModal.title == "Konfirmasi Gadai" ||
              currentModal.prompt.find("Konfirmasi Gadai") !=
                  std::string::npos)) {
    // Mortgage Interactions
    Rectangle noBtn = {dialogRect.x + 40,
                       dialogRect.y + dialogRect.height - 100,
                       (dialogRect.width - 100) / 2, 60};
    Rectangle yesBtn = {dialogRect.x + dialogRect.width / 2 + 10,
                        dialogRect.y + dialogRect.height - 100,
                        (dialogRect.width - 100) / 2, 60};

    if (GuiWindowInternal::isButtonPressed(yesBtn, true) ||
        IsKeyPressed(KEY_ENTER)) {
      std::lock_guard<std::mutex> lock2(modalMutex);
      modal.response.accepted = true;
      modal.response.value = "y";
      ...
      modal.active = false;
      modalCondition.notify_all();
    } else if (GuiWindowInternal::isButtonPressed(noBtn, true) ||
    ...
    return;
             ...
             (currentModal.title == "Konfirmasi Tebus" ||
              currentModal.prompt.find("Konfirmasi Tebus") !=
                  std::string::npos)) {
    // Redeem Interactions
    Rectangle noBtn = {dialogRect.x + 50,
                       dialogRect.y + dialogRect.height - 100,
                       (dialogRect.width - 120) / 2, 60};
    Rectangle yesBtn = {dialogRect.x + dialogRect.width / 2 + 10,
                        dialogRect.y + dialogRect.height - 100,
                        (dialogRect.width - 120) / 2, 60};
    ...
    return;
             ...
             currentModal.prompt.find("Pilih tile tujuan teleport") !=
                 std::string::npos) {
    // Teleport Interactions
    Rectangle inputRect = {dialogRect.x + (dialogRect.width - 180) / 2,
                           dialogRect.y + 190, 180, 80};
    Rectangle btnUp = {inputRect.x + inputRect.width - 30, inputRect.y + 10, 30,
                       30};
    Rectangle btnDown = {inputRect.x + inputRect.width - 30, inputRect.y + 40,
                         30, 30};
    ...
    Rectangle okBtn = {dialogRect.x + 40, dialogRect.y + dialogRect.height - 90,
                       (dialogRect.width - 100) / 2, 60};
    Rectangle cancelBtn = {dialogRect.x + dialogRect.width / 2 + 10,
    ...
    return;
             ...
             currentModal.prompt.find("Aksi lelang") != std::string::npos) {
    // Auction Interactions
    Rectangle cancelBtn = {dialogRect.x + 40,
                           dialogRect.y + dialogRect.height - 90,
                           (dialogRect.width - 100) / 2, 70};
    Rectangle okBtn = {dialogRect.x + dialogRect.width / 2 + 10,
                       dialogRect.y + dialogRect.height - 90,
                       (dialogRect.width - 100) / 2, 70};
    ...
    return;
             ...
             currentModal.title.find("Festival") != std::string::npos) {
    // Festival Interactions
    Rectangle cancelBtn = {dialogRect.x + dialogRect.width - 310,
                           dialogRect.y + dialogRect.height - 85, 140, 60};
    Rectangle okBtn = {dialogRect.x + dialogRect.width - 160,
                       dialogRect.y + dialogRect.height - 85, 120, 60};
    ...
    // Choice Interactions
    Rectangle inputRect = {dialogRect.x + 40, dialogRect.y + 210,
                           dialogRect.width - 80, 80};
    Rectangle btnUp = {inputRect.x + inputRect.width - 40, inputRect.y + 10, 40,
                       35};
    Rectangle btnDown = {inputRect.x + inputRect.width - 40, inputRect.y + 45,
                         40, 35};

    bool isThreeChoices =
        (currentModal.prompt.find("(1/2/3)") != std::string::npos);
    ...
    if (currentModal.localType == LocalDialogType::ErrorMessage) {
      const Rectangle errorOkRect{
          dialogRect.x + (dialogRect.width - 120.0F) / 2.0F,
      ...
      if (IsKeyPressed(KEY_ENTER) ||
          GuiWindowInternal::isButtonPressed(okRect, true)) {
        confirmLocalDialog();
      } else if (IsKeyPressed(KEY_ESCAPE) ||
                 GuiWindowInternal::isButtonPressed(cancelRect, true)) {
        cancelLocalDialog();
      }
    }
  }
