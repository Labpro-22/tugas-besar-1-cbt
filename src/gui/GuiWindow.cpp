...
GuiWindow::GuiWindow()
    : inputBuffer(), outputBuffer(), streamRedirector(), backendSession(),
      sessionThread(), snapshotMutex(), snapshot(), hasSnapshot(false),
      outputMutex(), outputText(), modalMutex(), modalCondition(), modal(),
      pendingErrorMutex(), pendingErrorPopups(), shuttingDown(false),
      exitRequested(false), inspectedPlayerIndex(-1), commandScrollColumn(0),
      commandScrollMaxColumn(0), logScrollLine(0), logAutoScroll(true),
      logScrollbarDragging(false), logScrollbarGrabOffset(0.0F),
      visibleCommandIndices{{-1, -1, -1, -1, -1, -1}},
      quickButtonLabels{{"", "", "", "", "", ""}},
      quickButtonEnabled{{false, false, false, false, false, false}},
      manualEnabled(false), gameOverPopupDismissed(false),
      lastAnnouncedActivePlayerIndex(-1), turnPopupPlayerIndex(-1),
      turnPopupPlayerName(), turnPopupTimer(0.0F), georgiaFont{},
      modalPosition{0.0F, 0.0F}, modalDragOffset{0.0F, 0.0F},
      modalDragging(false), modalPositionInitialized(false),
      modalScrollOffset(0.0F), modalScrollMax(0.0F) {}

GuiWindow::~GuiWindow() { stopSession(); }

int GuiWindow::run() {
  SetConfigFlags(FLAG_WINDOW_RESIZABLE | FLAG_MSAA_4X_HINT);
  InitWindow(GuiWindowInternal::kWindowWidth, GuiWindowInternal::kWindowHeight,
             "NIMONSPOLI GUI");
  SetTargetFPS(60);
  ...
  for (int fontIdx = 0; fontIdx < 10 && !fontLoaded; fontIdx++) {
    georgiaFont = GuiWindowInternal::loadSystemFontFromMemory(
        fontPaths[fontIdx], baseFontSize, nullptr, 0);
  ...
  backendSession.setSnapshotCallback([this](const GameSnapshot &nextSnapshot) {
    applySnapshot(nextSnapshot);
  });

  InputHandler::setPromptProvider([this](const InputPromptRequest &request) {
...
void GuiWindow::applySnapshot(const GameSnapshot &nextSnapshot) {
  ...
  if (nextSnapshot.gameStarted && nextSnapshot.activePlayerIndex >= 0 &&
