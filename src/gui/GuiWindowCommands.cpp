...
bool GuiWindow::isStartupExitChoice(const InputPromptRequest &request,
  ...
  if (!response.accepted ||
      GuiWindowInternal::trimWhitespace(response.value) != "0") {
    return false;
  }
...
void GuiWindow::updateQuickButtons(const GameSnapshot &currentSnapshot) {
  ...
  commandScrollColumn = GuiWindowInternal::clampInt(commandScrollColumn, 0,
  ...
  for (int column = 0; column < visibleColumns; ++column) {
    ...
    if (topIndex < totalCommands) {
      quickButtonLabels[static_cast<std::size_t>(topSlot)] =
          GuiWindowInternal::kStartedQuickActions[static_cast<std::size_t>(
                                                      topIndex)]
              .label;
...
void GuiWindow::executeStartedCommand(const int specIndex) {
  ...
  case StartedQuickAction::PrintLog:
    openLocalDialog(
        LocalDialogType::LogCount, "Cetak Log",
...
void GuiWindow::confirmLocalDialog() {
  ...
  const std::string value =
      GuiWindowInternal::trimWhitespace(current.inputText);
  ...
  case LocalDialogType::SetDice: {
    ...
    int d2 = 0;
    std::string extra;
    if (!(iss >> d1 >> d2) || (iss >> extra) || d1 < 1 || d1 > 6 || d2 < 1 ||
        d2 > 6) {
      std::lock_guard<std::mutex> lock(modalMutex);
...
InputPromptResponse
GuiWindow::requestBackendPrompt(const InputPromptRequest &request) {
  ...
  modalCondition.wait(
      waitLock, [this]() { return shuttingDown || modal.backendResolved; });
...
std::string
GuiWindow::currentStatusText(const GameSnapshot &currentSnapshot) const {
  if (!currentSnapshot.gameStarted) {
  ...
  if (inspectedIndex >= 0 &&
    ...
    return currentSnapshot.players[static_cast<std::size_t>(inspectedIndex)]
        .detailText;
  }
  ...
  if (modal.active) {
    if (!snapshot.gameStarted) {
      ...
      if (modal.prompt.find("[PROPERTY_PURCHASE:") != std::string::npos) {
        ...
        width = 1000.0F;
        height = 560.0F;
      } else if (modal.prompt.find("Pilih kartu yang ingin digunakan") !=
                 std::string::npos) {
        width = 620.0F;
        height = 580.0F;
      } else if (modal.prompt.find("Pilih target LassoCard") !=
                 std::string::npos) {
        width = 720.0F;
        height = 620.0F;
      } else if (modal.prompt.find("Pilih tile tujuan teleport") !=
                 std::string::npos) {
        width = 540.0F;
        height = 580.0F;
        ...
        width = 500.0F;
        height = 540.0F;
      } else if (modal.prompt.find("Masukkan kode properti") !=
                     std::string::npos &&
                 modal.title.find("Festival") != std::string::npos) {
        width = 500.0F;
        height = 340.0F;
        ...
        width = 600.0F;
        height = 650.0F;
      } else if (modal.title == "Pilih Opsi" ||
                 modal.prompt.find("Pilihan (1/") != std::string::npos) {
        width = 500.0F;
