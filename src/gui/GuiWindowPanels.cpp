#include "gui/GuiWindowPanels.hpp"
...
void GuiWindow::drawHeader(const Layout& layout,
    ...
    DrawRectangleRec(layout.headerRect, GuiWindowInternal::kPanel);
    DrawRectangleRec(
    ...
    GuiWindowInternal::drawTextCentered(font, "", layout.headerRect, 20.0F, 1.0F, GuiWindowInternal::kInk);
    DrawTextEx(font, "Nimonspoli",
               Vector2{layout.headerRect.x + 28.0F, layout.headerRect.y + 22.0F},
               30.0F, 1.0F, GuiWindowInternal::kAccentDark);
    ...
    DrawTextEx(font, navText.c_str(),
               ...
               18.0F, 1.0F, GuiWindowInternal::kMuted);
    ...
    if (currentSnapshot.gameStarted && currentSnapshot.activePlayerIndex >= 0 &&
        ...
        const std::string turnText =
            "Turn " + std::to_string(currentSnapshot.currentTurn) + ": Player " +
            std::to_string(currentSnapshot.activePlayerIndex + 1);
        ...
        GuiWindowInternal::drawTextCentered(font,
                         GuiWindowInternal::truncateText(font, turnText, 20.0F, 1.0F,
                                      turnRect.width - 12.0F),
                         turnRect, 20.0F, 1.0F, GuiWindowInternal::kAccentDark);
    }

    DrawTextEx(font, "[ ] #",
               Vector2{layout.headerRect.x + layout.headerRect.width - 72.0F,
                       layout.headerRect.y + 22.0F},
               26.0F, 1.0F, GuiWindowInternal::kAccentDark);
}

void GuiWindow::drawPanel(const Rectangle& rect, const std::string& title,
                          const std::string& subtitle) const {
    const Font& font = georgiaFont;
    GuiWindowInternal::drawPanelFrame(rect);

    if (!title.empty()) {
        const Rectangle titleRect = {rect.x + 18.0F, rect.y + 12.0F,
                                     rect.width - 36.0F, 70.0F};
        GuiWindowInternal::drawWrappedText(font, title, titleRect, 24.0F, 1.0F, GuiWindowInternal::kAccentDark, 2);
    }

    if (!subtitle.empty()) {
        DrawTextEx(font, subtitle.c_str(),
                   Vector2{rect.x + 18.0F, rect.y + 64.0F}, 18.0F, 1.0F, GuiWindowInternal::kMuted);
    }
}
...
void GuiWindow::drawStatusPanel(const Layout& layout,
    ...
    GuiWindowInternal::drawWrappedText(font, currentStatusText(currentSnapshot), layout.statusRect, 18.0F,
                    1.0F, GuiWindowInternal::kInk, 40);
    EndScissorMode();
}
...
void GuiWindow::drawRoster(const Layout& layout,
    ...
    if (currentSnapshot.players.empty()) {
        GuiWindowInternal::drawTextCentered(font, "Belum ada pemain aktif.", layout.rosterRect, 18.0F,
                         1.0F, GuiWindowInternal::kMuted);
        return;
    }
    ...
    for (std::size_t i = 0; i < currentSnapshot.players.size(); ++i) {
        ...
        Color cardColor = player.bankrupt ? GuiWindowInternal::kDisabled : GuiWindowInternal::kWhitePanel;
        Color textColor = GuiWindowInternal::kInk;
        Color detailColor = GuiWindowInternal::kMuted;
        if (isActive) {
            cardColor = GuiWindowInternal::kAccent;
            textColor = GuiWindowInternal::kWhitePanel;
            detailColor = GuiWindowInternal::kWhitePanel;
        } else if (isInspected) {
            cardColor = GuiWindowInternal::kInspected;
        }

        DrawRectangleRec(cardRect, cardColor);
        DrawRectangleLinesEx(cardRect, isInspected ? 2.0F : 1.0F,
                             isActive || isInspected ? GuiWindowInternal::kAccentDark : GuiWindowInternal::kPanelBorder);

        const Rectangle nameRect{cardRect.x + 14.0F, cardRect.y + 8.0F,
                                 cardRect.width - 28.0F, 24.0F};
        const std::string nameText =
            GuiWindowInternal::truncateText(font, player.name, compactRoster ? 18.0F : 22.0F, 1.0F,
                         nameRect.width);
        ...
        if (compactRoster) {
            ...
            summary << "Rp " << player.cash << " | " << GuiWindowInternal::playerStateLabel(player)
                    << " | " << player.propertyCount << "p | " << player.cardCount
                    << "k";
            const std::string summaryText =
                GuiWindowInternal::truncateText(font, summary.str(), 15.0F, 1.0F, cardRect.width - 28.0F);
            ...
            detail << GuiWindowInternal::playerStateLabel(player) << " | " << player.propertyCount
                   << " properti | " << player.cardCount << " kartu";
            const std::string detailText =
                GuiWindowInternal::truncateText(font, detail.str(), 14.0F, 1.0F, cardRect.width - 28.0F);
