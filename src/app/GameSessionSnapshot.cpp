...
void GameSession::notifySnapshotImmediate() { notifySnapshot(); }
...
GameSnapshot GameSession::buildSnapshot() const {
  ...
  for (int i = 0; i < board.getTileCount(); ++i) {
    ...
    tileSnapshot.code = i < static_cast<int>(layout.size())
    ...
    if (tile.getType() == "property") {
      ...
      if (property.getType() == "Street") {
        ...
        if (!street.getRentLevels().empty()) {
          tileSnapshot.rentPrice = street.getRentLevels()[0];
          tileSnapshot.rentLevels = street.getRentLevels();
        }
        tileSnapshot.buildingPrice = street.getHouseCost();
        tileSnapshot.houseCost = street.getHouseCost();
        tileSnapshot.hotelCost =
            street.getHouseCost(); // Assuming hotel relies on the same base
                                   // cost in Nimonspoli
      } else if (property.getType() == "Railroad") {
        const Railroad &rr = static_cast<const Railroad &>(property);
        if (!rr.getRentTable().empty()) {
          tileSnapshot.rentPrice = rr.getRentTable().begin()->second;
          for (const auto &kv : rr.getRentTable()) {
            tileSnapshot.rentLevels.push_back(kv.second);
          }
        }
      } else if (property.getType() == "Utility") {
        const Utility &ut = static_cast<const Utility &>(property);
        if (!ut.getMultiplierTable().empty()) {
          tileSnapshot.rentPrice = ut.getMultiplierTable().begin()->second;
          for (const auto &kv : ut.getMultiplierTable()) {
            tileSnapshot.rentLevels.push_back(kv.second);
          }
        }
      }
...
std::string GameSession::buildPlayerDetailText(const Player &player) const {
  ...
  if (player.getProperties().empty()) {
    ...
    for (Property *property : player.getProperties()) {
      ...
      displayed++;
      if (displayed >= 12 &&
          player.getProperties().size() > static_cast<std::size_t>(displayed)) {
        oss << "... +"
  ...
  if (player.getHand().empty()) {
    ...
    for (Card *card : player.getHand()) {
      ...
      oss << "- " << card->getDisplayLabel() << " | " << card->getDescription()
          << "\n";
    }
  }
