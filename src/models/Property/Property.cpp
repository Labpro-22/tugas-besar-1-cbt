...
Property::Property(std::string code, std::string name, PropertyStatus status, int mortgageValue, Player* owner)
    : code(code), name(name), status(status), mortgageValue(mortgageValue), owner(owner) {}
...
int Property::mortgage() {
    ...
    return 0; // Cannot mortgage if not owned or already mortgaged
}
