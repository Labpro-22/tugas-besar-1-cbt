...
template <typename T>
struct HasDrawCommunityChestCard<
    T, std::void_t<decltype(std::declval<T &>().drawCommunityChestCard())>>
    : std::true_type {};
...
template <typename T>
struct HasGetCommunityChestDeck<
    T, std::void_t<decltype(std::declval<T &>().getCommunityChestDeck())>>
    : std::true_type {};

template <typename T, typename = void> struct HasDeckDraw : std::false_type {};

template <typename T>
struct HasDeckDraw<T, std::void_t<decltype(std::declval<T &>().draw())>>
    : std::true_type {};
...
template <typename T>
struct HasDeckIsEmpty<
    T, std::void_t<decltype(std::declval<const T &>().isEmpty())>>
    : std::true_type {};
...
struct HasExecuteCommunityChestCard<
    GameType, CardType,
    std::void_t<decltype(std::declval<GameType &>().executeCommunityChestCard(
        std::declval<Player &>(), std::declval<CardType &>()))>>
    : std::true_type {};
...
template <typename GameType, typename CardType>
struct HasResolveCommunityChestCard<
    GameType, CardType,
    std::void_t<decltype(std::declval<GameType &>().resolveCommunityChestCard(
        std::declval<Player &>(), std::declval<CardType &>()))>>
    : std::true_type {};
...
struct HasCardExecute<
    CardType, std::void_t<decltype(std::declval<CardType &>().execute(
                  std::declval<Player &>(), std::declval<GameManager &>()))>>
    : std::true_type {};
...
template <typename CardType>
struct HasCardApply<
    CardType, std::void_t<decltype(std::declval<CardType &>().apply(
                  std::declval<Player &>(), std::declval<GameManager &>()))>>
    : std::true_type {};
...
template <typename GameType, typename CardType>
struct HasDiscardCommunityChestCard<
    GameType, CardType,
    std::void_t<decltype(std::declval<GameType &>().discardCommunityChestCard(
        std::declval<const CardType &>()))>> : std::true_type {};
...
struct HasDeckDiscardCard<
    DeckType, CardType,
    std::void_t<decltype(std::declval<DeckType &>().discardCard(
        std::declval<const CardType &>()))>> : std::true_type {};

template <typename CardType>
void executeCommunityChestCardEffect(Player &player, GameManager &game,
                                     CardType &card) {
  ...
  } else if constexpr (HasExecuteCommunityChestCard<GameManager,
                                                    CardType>::value) {
    game.executeCommunityChestCard(player, card);
  } else if constexpr (HasResolveCommunityChestCard<GameManager,
                                                    CardType>::value) {
...
template <typename CardHandle> auto *toCardPtr(CardHandle &card) {
